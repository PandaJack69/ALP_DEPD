...
class LombaTeamTable extends StatelessWidget {
  final Map<String, int> statusMap;
  final Function(String, int) onStatusChanged;

  const LombaTeamTable({super.key, required this.statusMap, required this.onStatusChanged});
  ...
  Widget build(BuildContext context) {
    return DataTable(
      ...
      rows: [
        _buildRow("Rian Wijaya", "UNAIR", "Alpha Tech", "2022", "08221133445", "UI/UX Design"),
      ],
    );
  }

  DataRow _buildRow(String name, String univ, String team, String year, String phone, String comp) {
    int currentStatus = statusMap[name] ?? 0;
    return DataRow(cells: [
      DataCell(Text(name, style: const TextStyle(fontWeight: FontWeight.bold))),
      DataCell(Text(univ)),
      DataCell(Text(team, style: const TextStyle(color: Colors.blue))),
      DataCell(Text(year)),
      DataCell(Text(phone)),
      DataCell(Text(comp)),
      DataCell(Row(
